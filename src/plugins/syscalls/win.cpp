...
get_syscall_retinfo(drakvuf_t drakvuf, drakvuf_trap_info_t* info, win_syscalls* s)
{
    ...
    {
        return { mode, resolve_module(drakvuf, ret, info->proc_data.base_addr, info->proc_data.pid, s) };
    }
    return { MAXIMUM_MODE, {} };
}
...
)
{
    ...
    std::optional<fmt::Qstr<std::string>> from_dll_opt;
    std::optional<fmt::Qstr<const char*>> priv_mode_opt;


    if (from_dll.has_value())
        from_dll_opt = from_dll.value();

    if (mode != MAXIMUM_MODE)
        priv_mode_opt = mode == USER_MODE ? "User" : "Kernel";
